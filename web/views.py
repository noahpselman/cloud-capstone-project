...
def annotate():
    ...
    key_name = '/'.join([app.config['AWS_S3_KEY_PREFIX'],
...
def create_annotation_job_request():
    ...
    try:
        ...
        sns_client = boto3.client(
            "sns", region_name=app.config['AWS_REGION_NAME'])
        sns_response = sns_client.publish(
            TopicArn=app.config['AWS_SNS_JOB_REQUEST_TOPIC'], Message=json.dumps(message_data))
    except ClientError as e:
...
def annotations_list():
    ...
    try:
        ...
        dynamo_client = boto3.client(
            'dynamodb', region_name=app.config['AWS_REGION_NAME'])
...
def annotation_details(id):
    ...
    try:
        ...
        item['submit_time'] = time.strftime(
            '%Y-%m-%d %H:%M:%S', time.localtime(int(float(item['submit_time']))))

        if item.get('complete_time'):
            item['complete_time'] = time.strftime(
                '%Y-%m-%d %H:%M:%S', time.localtime(int(float(item['complete_time']))))

        # hacky way to template rendering bug
        item['archive_status'] = item.get('archive_status', 'NOT_ARCHIVED')
        item['retrieval_status'] = item.get(
    ...
    if user_id != item['user_id']:
        ...
        app.logger.error(
            f"user_id doesn't match: {user_id} != {item['user_id']}")
        abort(403)
    ...
    if item['job_status'] == 'COMPLETED':
        ...
        try:
            s3_client = boto3.client(
                's3', region_name=app.config['AWS_REGION_NAME'])
